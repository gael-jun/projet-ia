...
(define (chainageArriere H BFinit BR)
  ...
  (define i 0)
  (if (and (member H BFini) (not (member 'fin lregls))) (set! lregls (append lregls (list (list H)))) (+ 1 2))
  (cond
                    ...
                    (else
                     ...
                     (if (and bool (not (member 'fin lregls))) (set! lregls (append lregls (list rule))) (+ 1 2))
                      ...
                      (if bool ; Si toutes les prémices ont été prouvées
                        (if (member 'fin lregls)
                            (begin
                              (set! lregl (remove 'fin lregls))
                              (list (reverse lregl) #t)) (list (reverse lregls) #t))
                        (loop (cdr rules)))))))
              
              
        )))))) ; H ne peut ni être prouvé ni être demandé, renvoyer #f

;(chainageArriere 'Paludisme '(Vomissement Fievre Maux_de_tête) '((Paludisme Vomissement Fievre Maux_de_tête) (COVID19 temperature_SUP_38 toux Perte_d’odorat)))
...
;(define new_BR (transformer-regle-en-liste "Paludisme,Vomissement,Fievre,Maux_de_tête!COVID19,temperature_SUP_38,toux,Perte_d’odorat"))
...
;(define new_BFinit (transformer-BF-en-liste "Vomissement!Maux_de_tête!Fievre"))


(define new_hypo (string->symbol (list-ref args 0)))

;(define new_hypo (string->symbol "Paludisme"))
